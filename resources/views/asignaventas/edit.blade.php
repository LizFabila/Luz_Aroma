                ...
                <div class="card">
                    <div class="card-header" style="background: rgba(255,51,153,.05);">
                        <h3 class="text-center" style="color: #333; font-size: 2.5rem; margin: 1rem 0;">
                            Editar <span style="color: #e84393;">Producto en Venta</span>
                        </h3>
                    </div>

                    <div class="card-body">
                        <form action="{{ route('asignaventas.update', [$asignaVenta->id_venta, $asignaVenta->id_asigna_ventas]) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <select name="id_producto" class="form-select @error('id_producto') is-invalid @enderror" required>
                                    ...
                                    @foreach($productos as $producto)
                                        <option value="{{ $producto->id_producto }}" {{ old('id_producto', $asignaVenta->id_producto) == $producto->id_producto ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="cantidad" class="form-control @error('cantidad') is-invalid @enderror"
                                       value="{{ old('cantidad', $asignaVenta->cantidad) }}" required min="1">
                                @error('cantidad')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" name="precio" class="form-control @error('precio') is-invalid @enderror"
                                       value="{{ old('precio', $asignaVenta->precio) }}" required min="0">
                                @error('precio')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="d-flex justify-content-between">
                                <a href="{{ route('asignaventas.index', $asignaVenta->id_venta) }}" class="btn btn-light" style="border: 2px solid #e84393; color: #000000;">Cancelar</a>
