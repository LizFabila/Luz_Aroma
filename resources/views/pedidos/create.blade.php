                ...
                <div class="card">
                    <div class="card-header" style="background: rgba(255,51,153,.05);">
                        <h3 class="text-center" style="color: #333; font-size: 2.5rem; margin: 1rem 0;">
                            Registrar <span style="color: #e84393;">Nuevo Pedido</span>
                        </h3>
                    </div>
                        ...
                        <form action="{{ route('pedidos.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" name="fecha_pedido" class="form-control @error('fecha_pedido') is-invalid @enderror"
                                       value="{{ old('fecha_pedido', now()->format('Y-m-d')) }}" required>
                                @error('fecha_pedido')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" name="total" class="form-control @error('total') is-invalid @enderror"
                                       value="{{ old('total') }}" required min="0">
                                @error('total')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="estado_pedido" class="form-select @error('estado_pedido') is-invalid @enderror" required>
                                    ...
                                    <option value="Completado" {{ old('estado_pedido') == 'Completado' ? 'selected' : '' }}>Completado</option>
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="submit" class="btn btn-light" style="border: 2px solid #e84393; background-color: rgba(246,170,208,0.39); color: #000000;">Registrar Pedido</button>
                            </div>
                        </form>
                    </div>
                </div>
